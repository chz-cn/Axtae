...
public sealed partial class Shelled : CharacterBody2D,
  ITakeDamage,
  IDropable {
  ...
  public float Speed { get; init; } = 10;

  private readonly uint _mask = (uint)Core.Rng.Shared.NextUInt64();
  public uint MaxHealth { get; init; } = 6;
  public uint Health {
  ...
  private readonly AnimatedSprite2D _body_sprite = new() {
    Frame = 0,
    Autoplay = Move,
    SpriteFrames = ResourceLoader.Load<SpriteFrames>(Url.Tres.Shelled),
  ...
  public override void _EnterTree() {
    ...
    this.AddChild(new CollisionShape2D {
      Shape = new CircleShape2D { Radius = 10 }
    });
