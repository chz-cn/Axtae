...
public abstract class RandomTests<T> where T : struct, IRandom, allows ref struct {
  ...
  public void NextUInt64_NotAllZero() {
    ...
    bool allZero = true;
    for (int i = 0; i < 20; i++)
      if (rng.NextUInt64() != 0) { allZero = false; break; }

    Assert.False(allZero, "All values were zero, algorithm likely broken.");
  }
  ...
  public void NextUInt64_DifferentSeeds_DifferentSequence() {
    ...
    Assert.True(anyDiff, "Different seeds produced identical sequence.");
  }
